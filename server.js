...
async function api(req,res,url){const method=req.method, p=url.pathname, db=readDb();
 ...
 if(method==='GET'&&p==='/api/companies'){const u=authorize(req,res);if(!u)return;const view=url.searchParams.get('view')|| (u.role==='admin' ? 'available' : 'assigned');const assignedUserId=url.searchParams.get('assignedUserId')|| (u.role==='admin' ? null : u.id);const companies=(db.companies||[]).filter(c=>companyMatchesAssignmentView(c, view, u.role==='admin' ? assignedUserId : u.id));return json(res,200,{companies:u.role==='admin'?companies:companies.filter(c=>(u.companyIds||[]).map(String).includes(String(c['#'])))})}
 if(method==='POST'&&p==='/api/companies/assign'){
  if(!authorize(req,res,true))return;const d=await body(req);if(!Array.isArray(d.companyIds))return json(res,400,{error:'Company IDs array required'});const assignedUserId=normalizeAssignedUserId(d.assignedUserId);if(assignedUserId){const target=db.users.find(x=>x.id===assignedUserId);if(!target)return json(res,404,{error:'User not found'})}assignCompanies(db,d.companyIds,assignedUserId);writeDb(db);return json(res,200,{ok:true,companies:db.companies})
 }
 if(method==='PUT'&&p==='/api/companies'){
  ...
  if(u.role==='admin'){db.companies=d.companies.map(c=>({...c,assigned_user_id:normalizeAssignedUserId(c?.assigned_user_id)}));syncUserCompanyIds(db);writeDb(db);return json(res,200,{ok:true,companies:db.companies})}
  ...
  const live=db.users.find(x=>x.id===u.id);live.companyIds=[...allowed];for(const company of db.companies){if(allowed.has(String(company['#'])))company.assigned_user_id=u.id;else if(company.assigned_user_id===u.id)company.assigned_user_id=null}syncUserCompanyIds(db);writeDb(db);return json(res,200,{ok:true,companies:db.companies.filter(c=>allowed.has(String(c['#'])))});
 }
